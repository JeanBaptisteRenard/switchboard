...
Middle-click pastes the X11/Wayland PRIMARY selection — the text you last selected with the mouse, which is separate from the `Ctrl+C` clipboard. Switchboard cancels the browser's default insertion so the selection is delivered to the terminal exactly once; left alone, a mouse paste arrives twice.
