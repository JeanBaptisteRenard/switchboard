// Regression coverage for the `auto` permission mode picker.
//
// Claude Code's CLI recognizes `auto` as a `--permission-mode` value
// (verified live: `claude -p ... --permission-mode auto` exits 0) and, per
// the CLI's own onboarding copy, it is now Claude Code's own default
// permission mode. Switchboard's three pickers (New Session dialog, Resume
// Session dialog, Settings panel <select>) previously omitted it entirely
// — this test locks in that it is offered everywhere, that it round-trips
// into the spawned session's options (which main.js turns directly into
// `--permission-mode "auto"`), and that the two dialogs.js copies of the
// mode list stay deduplicated behind the shared PERMISSION_MODES constant.
