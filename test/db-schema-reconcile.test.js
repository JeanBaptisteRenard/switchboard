...
test('foreign higher-version database is reconciled, not crashed', () => {
  ...
  try {
    ...
    // Fork divergence from upstream: our migrations array has 9 entries (vs 4
    // upstream), so a version-5 DB legitimately runs v6-v9 (all idempotent
    // try/catch) and gets bumped to 9. The invariant that matters is "never
    // downgraded"; a DB claiming a version above ours stays untouched.
    assert.equal(state.version, '9', 'foreign db_version upgraded to ours, never downgraded');
