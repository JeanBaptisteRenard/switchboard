...
- **`search` IPC is a synchronous main-thread call, not routed through a worker.** `ipcMain.handle('search', ...)` calls `searchByType()` directly, which runs the `better-sqlite3` FTS5 `MATCH` query synchronously on the Electron main process. Because Electron's main process is single-threaded, a slow query (large trigram-tokenized corpus, or a pathological query string) blocks IPC dispatch for the whole app — the renderer appears frozen (no terminal input, no window resize) until the query returns. There is currently no cap on `query` length before it reaches FTS5. If you touch search, prefer: run the query off the main thread (worker thread or `populateCacheViaWorker`-style pattern), cap the incoming query length before it's escaped/quoted, and guard against pathological patterns (e.g. long strings that are mostly repeated characters can pin CPU in the trigram tokenizer).
